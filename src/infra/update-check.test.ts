...
describe("resolveNpmChannelTag", () => {
      ...
      try {
        ...
        expect(
          requests.some((request) =>
            decodeURIComponent(request.url).startsWith("/user/@bitplanet/genie-openclaw"),
          ),
        ).toBe(true);
  ...
  it("uses the public registry when no npm command is available", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      "https://registry.npmjs.org/%40bitplanet%2Fgenie-openclaw/latest",
  ...
  it("uses a custom package name when provided", async () => {
    const fetch = vi.fn(async () =>
      Response.json({ version: "1.0.0", engines: { node: ">=22.19.0" } }),
    );
    vi.stubGlobal("fetch", fetch);
    ...
    expect(fetch).toHaveBeenCalledWith(
      "https://registry.npmjs.org/%40mycorp%2Fcustom-pkg/latest",
      expect.objectContaining({ signal: expect.any(AbortSignal) }),
    );
  });
