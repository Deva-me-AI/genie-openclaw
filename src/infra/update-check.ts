...
function packageTargetSpec(params: {
  ...
  return (
    spec ||
    `${params.packageName?.trim() || DEFAULT_PACKAGE_NAME}@${params.target.trim() || "latest"}`
  );
}

const PUBLIC_NPM_REGISTRY_URL = "https://registry.npmjs.org/";
const PUBLIC_NPM_PACKAGE_NAME = "openclaw";
const DEFAULT_PACKAGE_NAME = "@bitplanet/genie-openclaw";
  ...
  try {
    res = await fetchWithTimeout(
      npmRegistryTargetUrl({
        ...
        packageName: params.packageName ?? DEFAULT_PACKAGE_NAME,
  ...
  try {
    const res = await runCommand(
      [
        params.command ?? "npm",
        "view",
        packageTargetSpec({ target, spec: params.spec, packageName: params.packageName }),
        "version",
        "engines.node",
        "--json",
        "--global",
...
export async function checkUpdateStatus(params: {
  ...
  const timeoutMs = params.timeoutMs ?? 6000;
  const fetchRegistry = (packageName: string) =>
  ...
  if (!root) {
    return {
      ...
      registry: params.includeRegistry ? await fetchRegistry(DEFAULT_PACKAGE_NAME) : undefined,
    };
  }

  const rootRealpath = await fs.realpath(root).catch(() => root);
  const [pm, gitRoot, packageName] = await Promise.all([
    detectPackageManager(root),
    detectGitRoot(root),
    readPackageName(root).then((name) => name ?? DEFAULT_PACKAGE_NAME),
  ]);
  ...
  const registry = params.includeRegistry
    ? params.registryChannel === "extended-stable" && isGit
      ...
      : await fetchRegistry(packageName)
    : undefined;
