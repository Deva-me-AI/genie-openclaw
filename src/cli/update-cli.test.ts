...
const { DEFAULT_PACKAGE_NAME, ensureGitCheckout, resolveGitInstallDir } = updateCliShared;
...
describe("update-cli", () => {
  ...
  const mockGitInstallStatus = (root: string, options?: { resolveRoot?: boolean }) => {
    if (options?.resolveRoot !== false) {
      vi.mocked(resolveOpenClawPackageRoot).mockResolvedValue(root);
    }
    vi.mocked(checkUpdateStatus).mockResolvedValue({
      root,
  ...
  const stripOpenClawPackageAlias = (spec: string) => {
    ...
    const aliases = ["openclaw", DEFAULT_PACKAGE_NAME];
    const alias = aliases.find((candidate) =>
      trimmed.toLowerCase().startsWith(`${candidate.toLowerCase()}@`),
    );
    return alias ? trimmed.slice(alias.length + 1) : trimmed;
  };
  ...
  const mockOwnedGitService = () => {
    mockGitInstallStatus(process.cwd());
  ...
  beforeEach(() => {
    ...
    mockGitInstallStatus("/test/path", { resolveRoot: false });
  ...
  it.each([
    ...
    async ({ mode, options, prepare, expectedChannel, expectedTag, expectedPersistedChannel }) => {
      ...
      if (expectedChannel !== undefined) {
        ...
        expectPackageInstallSpec(`${DEFAULT_PACKAGE_NAME}@latest`);
      }
  ...
  it("warns but still runs package updates when disk space looks low", async () => {
    ...
    expect(preflightParams).toEqual(
      expect.objectContaining({
        target: "latest",
        spec: "@bitplanet/genie-openclaw@latest",
        cwd: process.cwd(),
      }),
    );
  ...
  it.each([
    {
      ...
      expectedSpec: `${DEFAULT_PACKAGE_NAME}@next`,
    },
    {
      name: "main shorthand",
      run: async () => {
  ...
  it("fails package updates when the installed correction version does not match the requested target", async () => {
    ...
    const pkgRoot = path.join(nodeModules, ...DEFAULT_PACKAGE_NAME.split("/"));
    mockPackageInstallStatus(tempDir);
    readPackageName.mockResolvedValue(DEFAULT_PACKAGE_NAME);
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: DEFAULT_PACKAGE_NAME, version: "2026.3.23" }),
      "utf-8",
    );
    ...
    const installCall = vi
      ...
      .mock.calls.find(
        ([argv]) =>
          ...
          argv[1] === "i" &&
          argv[2] === "-g" &&
          argv.includes(`${DEFAULT_PACKAGE_NAME}@latest`),
      );
