...
describe("ci workflow guards", () => {
  ...
  it("skips automatic Claude review when the review workflow changed", () => {
    ...
    const reviewJob = workflow.jobs["claude-review"];
    expect(reviewJob.if).toContain("github.event.comment.author_association == 'OWNER'");
    expect(reviewJob.if).toContain("github.event.comment.author_association == 'MEMBER'");
    expect(reviewJob.if).toContain("github.event.comment.author_association == 'COLLABORATOR'");
    ...
    expect(trustStep.if).toBeUndefined();
    expect(trustStep.run).toContain(
      'gh api "repos/${GITHUB_REPOSITORY}/pulls/${PULL_REQUEST_NUMBER}" --jq .base.sha',
    );
    expect(trustStep.run).toContain(
    ...
    expect(reviewStep.if).toBe("steps.workflow-trust.outputs.workflow_matches_base == 'true'");
  });
